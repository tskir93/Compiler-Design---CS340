...
struct boolList * truelist_head;
struct boolList * falselist_head;
struct boolList * temp_list;
...
expr			:	assignexpr						{$$=$1;	printf("expr <- assignexpr\n");}
				|	expr PLUS expr				{ 
													if(check_corr_type($1)==0){
														...
														pushlist(truelist_head,nextquad());	//meriki extra
														pushlist(falselist_head,nextquad()+1);
				...
				|	expr LESS_THAN expr				{
														if(check_corr_type($1)==0){
															...
															emit(jump,NULL,NULL,NULL,nextquad()+3,yylineno);		//tskir: egw to prosthesa giati den to ekane print
				...
				|	expr NOTEQUAL expr				{
														if(check_corr_type($1)==0){
															...
															emit(assign,t,NULL,$$,0,yylineno);
															printf("expr <- expr op expr\n");
															temp_list=truelist_head;	//meriki apotimiso extra
															truelist_head=$3->falselist_head;
															falselist_head=$3->truelist_head;
															if($1!=$3){												//tskir: added last 1/5
																$$->val.boolConst = 0;
															}else{
																$$->val.boolConst= 1;
				...
				|	expr AND M expr				{
													...
													/*}
														if(getFirst(truelist_head)!=0){	//meriki apotimisi extra
                                                        patchlabel(getFirst(truelist_head),M.quad);
                                                        }
														truelist_head=$4->truelist_head;
														falselist_head=mergelists($1->falselist_head,$4->falselist_head);
													else{
																printf("\033[1;31m");
				...
				|	expr OR	M expr				{
													//if(check_corr_type($1)==0){
														...
														printf("expr <- expr op expr\n");
													/*}
														if(getFirst(falselist_head)!=0){	//meriki apotimisi extra
                                                        patchlabel(getFirst(falselist_head),M.quad);
                                                        }
														truelist_head=mergelists($1->truelist_head,$4->truelist_head);
														falselist_head=$4->falselist_head;
													else{
																printf("\033[1;31m");
...
int main(int argc, char **argv) {
	...
	truelist_head =malloc(sizeof(struct boolList));
	falselist_head=malloc(sizeof(struct boolList));
	Symtbl = new_Symtable();
	makestack();
