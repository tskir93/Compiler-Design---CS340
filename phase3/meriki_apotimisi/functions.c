...
bool true_test(expr * ex){
    switch(ex->type){
        case var_e:
        if(ex->val.numConst!=0)
        return true;
        return false;
        ...
        case conststring_e:
        if(ex->val.strConst!= NULL && strcmp(ex->val.strConst,"")!=0)
        return true;
        return false;
...
boolList * merge_lists(boolList a,boolList b){    //ennonoume dio listes
    boollist_node * temp_a;
    temp_a=a.List;
    while(temp_a!=NULL){    //briskoume ton to teleutaio stoixeio tis protis listas
        temp_a=temp_a->next;
    }
    temp_a->next=b.List;    // kai to ennonoume me tin proti
