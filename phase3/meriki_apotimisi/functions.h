...
typedef struct boollist_node{ //struct gia truelist kai breaklist
    int val;
    struct boollist_node * next;
}boollist_node;

typedef struct boolList{
  struct boollist_node * List;  
}boolList;
...
typedef struct expr{
    ...
    struct expr* next;
    boolList * truelist_head;
    boolList  * falselist_head;
}expr;
...
boolList * merge_lists(boolList a,boolList b);
